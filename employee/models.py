...
Handles Attendance (daily check-in/out) and Leave requests.
"""
import base64, uuid
...
def _save_b64_photo(b64_string: str, upload_to: str) -> str | None:
    """
    Converts a base64 dataURL (from JS canvas.toDataURL)
    to a real file on disk and returns the relative path.
    Returns None if the string is empty/invalid.
    """
...
class Attendance(models.Model):
    ...
    # Face photos captured at check-in / check-out
    ...
    def duration(self) -> str | None:
        """Work duration as human-readable string, e.g. '8j 15m'."""
    ...
    def duration_minutes(self) -> int:
        """Work duration in minutes (0 if not complete)."""
        if self.check_in and self.check_out:
